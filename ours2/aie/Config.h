...
constexpr int kOutputLanesPerPlio = 1;
...
static_assert(kTopPlWorkers == 4,
              "ours2 worker-Q0 validation build expects four PL workers");
static_assert(kParallelLanes == 16,
              "ours2 worker-Q0 validation build expects sixteen AIE lanes");
...
static_assert(kWorkerLanes == 4,
              "ours2 maps four AIE lanes to each PL worker");
static_assert(kWorkerOutputGroups == kWorkerLanes,
              "Each worker must own one output PLIO group per lane");
...
static_assert(kTotalTileCount == kParallelLanes,
              "ours2 64x64 validation maps one 16x16 tile to each lane");
