...
// Generated Ours2 SRAD AIE graph:
//   16 lanes over a 4x4 16x16 tile grid for 64x64 input.
...
public:
    ...
    static_assert(kNumLanes == 16,
                  "ours2 generated graph expects sixteen lanes");
    static_assert(srad_cfg::kAieArrayRows == 8,
                  "ours2 generated graph expects eight AIE rows");
    static_assert(srad_cfg::kKernelsPerParallelLane == 2,
                  "ours2 lanes use K1/K2 adjacent kernels");
...
public:
    ...
    output_plio out_j_next[kNumLanes];
    ...
    GraphOursPLQ0(const std::string& graphID) {
        for (int lane = 0; lane < kNumLanes; ++lane) {
            ...
            out_j_next[lane] = output_plio::create(
                graphID + "_out_j_next_" + std::to_string(lane),
                plio_64_bits,
                srad_plio_files::j_next_out(lane));

            connect<>(in_j[lane].out[0], core.in_j_k1[lane]);
            connect<>(in_j[lane].out[0], core.in_j_k2[lane]);
            connect<>(core.out_j_next[lane], out_j_next[lane].in[0]);
        }
    }
};
