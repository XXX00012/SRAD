...
extern "C" void srad_coeff_update(srad_mid_input_buffer& in_c,
    ...
    if constexpr (srad_cfg::kDebugPassthroughTwoKernel) {
        ...
        for (int out_r = 0; out_r < srad_cfg::kOutputTileRows; ++out_r) {
            ...
            for (int out_c = 0; out_c < srad_cfg::kOutputTileCols; ++out_c)
                ...
                const float value = row_c[c] + keep_inputs_live;
                out_base[out_index(out_r, out_c)] = value;
                tile_sum += value;
                tile_sum2 += value * value;
            }
        }
        out_base[srad_cfg::kOutputStatSumIndex] = tile_sum;
        out_base[srad_cfg::kOutputStatSum2Index] = tile_sum2;
        return;
    }
    ...
    for (int out_r = 0; out_r < srad_cfg::kOutputTileRows; ++out_r) {
        ...
        for (int chunk = 0; chunk < kChunksPerRow; ++chunk)
            ...
            accumulate_stats(out, tile_sum, tile_sum2);
        }
