...
// Generated Ours16 SRAD AIE graph:
//   20 lanes over a 10x10 16x16 tile grid for 160x160 input.
...
// padding column of the input tile, so no separate q0 PLIO is required.
...
public:
    ...
    static_assert(kNumLanes == 20,
                  "ours_16 generated graph expects 20 lanes");
    static_assert(srad_cfg::kAieArrayRows == 8,
                  "ours_16 generated graph expects 8 AIE rows");
    static_assert(srad_cfg::kKernelsPerParallelLane == 2,
                  "ours_16 lanes use K1/K2 adjacent kernels");
    ...
    SradCoreGraph() {
        ...
        for (int lane = 0; lane < kNumLanes; ++lane) {
            ...
            stack_size(k_local_q[lane]) = 8192;
            stack_size(k_coeff_update[lane]) = 8192;
    ...
    GraphOursPLQ0(const std::string& graphID) {
        ...
        for (int lane = 0; lane < kNumLanes; ++lane) {
            ...
            connect<window<srad_cfg::kOutputSampleBytes>, pktstream>(
                core.out_j_next[lane],
                out_merge[lane / kOutputLanesPerPlio]
                    .in[lane % kOutputLanesPerPlio]);
        }

        for (int group = 0; group < kOutputGroups; ++group) {
            connect<pktstream>(out_merge[group].out[0],
                               out_j_next[group].in[0]);
        }
    }
};
