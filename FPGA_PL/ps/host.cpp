// FPGA-v5-style SRAD baseline with PL q0sqr:
//   DDR -> PL q0/source shim -> AIE phase2 -> PL sink shim -> DDR.
//
// All PLIO-visible SRAD data is float32, matching Rodinia OpenCL SRAD's
// `fp float`. The only semantic difference from SRAD/FPGA is that q0sqr is
// computed in PL instead of inside the AIE kernel.
...
struct PipelineTiming {
    ...
    long long wait_source_us = 0;
    long long wait_aie_graph_us = 0;
...
void print_wall_timing(const char* name, const PipelineTiming& timing) {
    ...
    std::printf("%s hw_emu wall timing us: submit=%lld wait_inputs=%lld wait_output=%lld wait_graph=%lld total=%lld\n",
                ...
                total);
    std::printf("%s hw_emu split us: source_plio_including_pl_q0=%lld aie_phase2=%lld combined=%lld\n",
                name,
                timing.wait_source_us,
                timing.wait_aie_graph_us,
                timing.wait_source_us + timing.wait_graph_us);
}
...
int main(int argc, char* argv[]) {
    ...
    if (iter_cnt != 1) {
        std::fprintf(stderr,
                     "[error] this FPGA_PL baseline supports one iteration; got %d\n",
                     iter_cnt);
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        auto load_fpga = open_kernel_or_die(device, xrt_uuid, "LoadFpgaV5PLQ0");
        auto store_fpga = open_kernel_or_die(device, xrt_uuid, "StoreFpgaV5PLQ0");
        ...
        std::printf("---- LoadFpgaV5PLQ0 + GraphFpgaV5PLQ0 + StoreFpgaV5PLQ0: PL q0 + AIE phase2 ----\n");
        ...
        source_run.wait();
        timing.wait_source_us = elapsed_us(stage_t0, Clock::now());
        timing.wait_inputs_us += timing.wait_source_us;

        stage_t0 = Clock::now();
        graphFpgaV5PLQ0.wait();
        timing.wait_aie_graph_us = elapsed_us(stage_t0, Clock::now());
        timing.wait_graph_us += timing.wait_aie_graph_us;
        ...
        std::printf("PL q0 + AIE phase2 time: %lld us\n", static_cast<long long>(dur_us));
        print_wall_timing("GraphFpgaV5PLQ0", timing);

        CommonTiming common;
        common.data_transfer_us =
            timing.submit_us + timing.wait_inputs_us + timing.wait_output_us;
        common.logic_compute_us = timing.wait_graph_us;
        common.end_to_end_us = dur_us;
        print_common_timing("FPGA_PL", common);
