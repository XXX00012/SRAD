...
def read_config_dims():
    ...
    if rows is None or cols is None:
        raise RuntimeError(f"cannot read kRows/kCols from {cfg}")
    return int(rows.group(1)), int(cols.group(1))


ROWS, COLS = read_config_dims()
PIXELS = ROWS * COLS
LAMBDA = 0.5
...
def main():
    ...
    write_vector(base / "plio_fpga_pl_compute.txt", image)
    write_vector(base / "plio_fpga_pl_q0sqr.txt", [q0sqr] + [0.0] * 7)
    write_vector(base / "plio_fpga_pl_lambda.txt", [LAMBDA] + [0.0] * 7)
    q0_packet = "\n".join(f"{q0sqr:.9g}" if i == 0 else "0" for i in range(8)) + "\n"
    ...
    print(f"generated {ROWS}x{COLS} FPGA_PL float SRAD case")
