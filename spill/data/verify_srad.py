        ...
        for tile_c in range(cfg["kTileColCount"]):
            ...
            for local_row in range(cfg["kOutputTileRows"]):
                ...
                for local_col in range(cfg["kOutputTileCols"]):
                    ...
                    raw.append(compact[out_r * output_cols + out_c])
    return raw
...
def normalize_aie_output(values: list[float],
    ...
    raw_tile_elems = cfg["kGraphRunIterations"] * cfg["kOutputSampleElems"]
    if len(values) == raw_tile_elems:
    ...
    raise RuntimeError(
        ...
        f"or {raw_tile_elems} raw tile-stream"
    )
...
def main() -> int:
    ...
    gold_tile = compact_to_raw_tile_stream(gold, cfg)
    ...
    print(f"graph firings  : {cfg['kGraphRunIterations']}")
