...
int main() {
    ...
    std::printf("[aiesim] GraphOursUndividePLQ0: %d graph firing(s), %dx%d floats per firing, fused full-tile pressure block on 32x32 data\n",
                srad_cfg::kBlockCount,
                srad_cfg::kBlockRows,
                srad_cfg::kBlockCols);
    std::printf("[aiesim] stack experiment: 16x16 data, 16x16 tile, fused 4-row pressure group; one graph firing moves and updates 256 floats while SIMD math uses v8float lanes\n");
