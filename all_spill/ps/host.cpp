...
//   fused tiles: compute local JC/C/dN/dS/dW/dE for all 16 rows first, then
//   update the whole 16x16 tile without materializing the mid plane.
//
...
int main(int argc, char* argv[]) {
    ...
    std::printf("stack experiment      : 16x16 data, 16x16 tile, fused 4-row pressure group, full 256-point update per firing\n");
    ...
    try {
        ...
        std::printf("AIE fused             : srad_undivide_fused computes C and updates one row per pressure group while one graph firing still covers all 256 points\n");
