...
constexpr int kAieArrayRows = 2;
constexpr int kParallelLanes = 4;
...
static_assert(kTopPlWorkers == 4,
              "ours worker-Q0 validation build expects four PL workers");
static_assert(kParallelLanes == 4,
              "ours worker-Q0 validation build expects four AIE lanes");
...
static_assert(kWorkerLanes == 1,
              "ours validation maps one lane to each PL worker");
static_assert(kWorkerOutputGroups == 1,
              "ours validation maps one output group to each PL worker");
