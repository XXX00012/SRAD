// CUDA_PL staged AIE reproduction:
//   PL TopPL q0sqr reduction + DDR-to-PLIO source -> GraphCoeff -> GraphUpdate
//
// All PLIO-visible data is float32, matching Rodinia CUDA SRAD's `fp float`.
// This file intentionally mirrors SRAD/CUDA's tiled host flow after q0sqr.
// The first-stage semantics are the same as SRAD/CUDA's GraphReduce: full-image
// column-major materialization plus OpenCL-v0 256-lane tree reduction. CUDA_PL
// executes that stage inside PL TopPL and streams q0sqr to GraphCoeff.
...
float compute_q0sqr_reference(const std::vector<float>& image) {
    ...
    auto ceil_div_int = [](int x, int y) {
        return (x + y - 1) / y;
    };

    auto last_opencl_block_count = [](int no, int grid_dim) {
        return srad_cfg::kOpenClReductionThreads -
               (grid_dim * srad_cfg::kOpenClReductionThreads - no);
    };

    auto largest_opencl_reduction_pow2 = [](int n) {
        int df = 1;
    ...
    auto reduce_full_opencl_block = [](float* psum, float* psum2) {
        for (int i = 2; i <= srad_cfg::kOpenClReductionThreads; i *= 2) {
    ...
    auto opencl_v0_reduce_stats_reference = [&]() {
        ...
        return OpenClReduceStats{sums[0], sums2[0]};
    };

    const OpenClReduceStats stats = opencl_v0_reduce_stats_reference();
    const float mean = stats.sum / static_cast<float>(srad_cfg::kPixels);
    const float mean2 = mean * mean;
    const float variance =
        (stats.sum2 / static_cast<float>(srad_cfg::kPixels)) - mean2;
...
int main(int argc, char* argv[]) {
    ...
    auto top_pl = open_kernel_or_die(device, xrt_uuid, "TopPL");
    ...
    auto image_bo = xrt::bo(device, srad_cfg::kImageBytes, top_pl.group_id(0));
    ...
    std::printf("---- TopPL + GraphCoeff: DDR -> PL(q0) -> PLIO -> AIE ----\n");
    std::fflush(stdout);
    graphCoeff.init();
    std::printf("[host] TopPL computes one q0 and GraphCoeff consumes %d tiles\n",
                n_tiles);
    std::fflush(stdout);
    auto stage_t0 = Clock::now();
    auto sink_run = store_coeff(c_bo, dN_bo, dS_bo, dW_bo, dE_bo);
    graphCoeff.run(n_tiles);
    auto source_run = top_pl(image_bo);
    source_run.wait();
    ...
    std::printf("q0sqr_pl path    : computed by TopPL and streamed to GraphCoeff through PLIO\n");
    ...
    std::fflush(stdout);
    stage_t0 = Clock::now();
    auto update_sink_run = store_update(out_bo);
    ...
    common.logic_compute_us =
        timing.coeff_compute_us + timing.update_compute_us;
    common.end_to_end_us = dur_us;
    print_common_timing("CUDA_PL", common);
