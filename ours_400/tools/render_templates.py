...
def tokenize_template(text: str) -> list[str]:
    ...
    for idx, token in enumerate(tokens):
        ...
        # Match the Jinja environment used below: lstrip_blocks=True and
        # trim_blocks=True. This keeps fallback-rendered files stable on
        # machines without jinja2 installed.
...
def read_design(path: Path) -> dict[str, object]:
    ...
    required = {
        ...
        "worker_group_lanes",
        "parallel_lanes",
        "input_lanes_per_plio",
        "output_lanes_per_plio",
        "aie_rows",
        "aie_cols",
        "kernels_per_lane",
        "srad_iterations",
    ...
    input_lanes_per_plio = int(design["input_lanes_per_plio"])
    output_lanes_per_plio = int(design["output_lanes_per_plio"])
    ...
    if parallel_lanes % input_lanes_per_plio != 0:
        raise RuntimeError(
            "parallel_lanes must be divisible by input_lanes_per_plio "
            f"({parallel_lanes} % {input_lanes_per_plio} != 0)"
        )
    ...
    output_plio_groups = parallel_lanes // output_lanes_per_plio
    if output_plio_groups % toppl_workers != 0:
        raise RuntimeError(
            "output PLIO groups must split evenly across TopPL workers "
            f"({output_plio_groups} % {toppl_workers} != 0)"
        )
    worker_output_groups = output_plio_groups // toppl_workers
    expected_worker_output_groups = worker_lanes // output_lanes_per_plio
    if worker_output_groups != expected_worker_output_groups:
        raise RuntimeError(
            "worker_output_groups must match worker_lanes/output_lanes_per_plio "
            f"({worker_output_groups} != {expected_worker_output_groups})"
        )
    ...
    design["worker_output_groups_list"] = list(range(worker_output_groups))
    design["output_group_lanes_list"] = list(range(output_lanes_per_plio))
    ...
    design["worker_output_group_pairs"] = [
        {
            "worker": worker,
            "local_group": local_group,
            "global_group": worker * worker_output_groups + local_group,
        }
        for worker in design["workers"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Render ours_400 Jinja templates.")
