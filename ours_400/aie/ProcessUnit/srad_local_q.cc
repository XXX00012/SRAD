...
constexpr unsigned kBroadcastLane0 = 0x00000000;
...
inline v8float load_cached_chunk(const CachedJRow& row, int chunk) {
    return *(reinterpret_cast<const v8float*>(
        row.lane + chunk * srad_cfg::kLanes));
}
...
template <int C>
inline v8float load_cached_halo_vec(const CachedJRow& row, v8float one) {
    ...
    if constexpr ((C & (srad_cfg::kLanes - 1)) == 0) {
        return load_cached_chunk(row, C / srad_cfg::kLanes);
    }

    constexpr int kChunk = C / srad_cfg::kLanes;
    constexpr int kOffset = C & (srad_cfg::kLanes - 1);
    const v16float row_window =
        concat(load_cached_chunk(row, kChunk),
               load_cached_chunk(row, kChunk + 1));
    return fpmul(row_window, kOffset, kLaneOffsets,
                 one, 0, kBroadcastLane0);
}
...
inline v8float shift_left_one(v8float lo, v8float hi, v8float one) {
    const v16float window = concat(lo, hi);
    return fpmul(window, 1, kLaneOffsets, one, 0, kBroadcastLane0);
}
...
inline void compute_coeff_vec_at(const CachedJRow& row_n,
    ...
    const v8float j_c = load_cached_halo_vec<C>(row_c, one);
    const v8float j_n = load_cached_halo_vec<C>(row_n, one);
    const v8float j_s = load_cached_halo_vec<C>(row_s, one);
    const v8float j_w = load_cached_halo_vec<C - 1>(row_c, one);
    const v8float j_e = load_cached_halo_vec<C + 1>(row_c, one);
...
extern "C" void srad_local_q(srad_image_input_buffer& in_j,
    ...
    for (int out_r = srad_cfg::kOutputTileRows - 1; out_r >= 0; --out_r) {
        ...
        const v8float east_tail_value = splat(east_tail.value);
        const v8float east_tail_tag = splat(east_tail.tag);
        const v8float east_value0 =
            shift_left_one(row_value0, row_value1, one);
        const v8float east_tag0 =
            shift_left_one(row_tag0, row_tag1, one);
        const v8float east_value1 =
            shift_left_one(row_value1, east_tail_value, one);
        const v8float east_tag1 =
            shift_left_one(row_tag1, east_tail_tag, one);

        store_vec(east_value_base, out_idx0, east_value0);
        store_vec(east_tag_base, out_idx0, east_tag0);
        store_vec(east_value_base, out_idx1, east_value1);
        store_vec(east_tag_base, out_idx1, east_tag1);
