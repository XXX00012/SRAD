...
xrt::kernel open_kernel_cu(const xrt::device& device,
    ...
    std::fprintf(stderr, "[diag] opening CU scoped name: %s\n", scoped.c_str());
    std::fflush(stderr);
    try {
        auto kernel = xrt::kernel(device, uuid, scoped.c_str());
        std::fprintf(stderr, "[diag] opened CU: %s\n", scoped.c_str());
        std::fflush(stderr);
        return kernel;
    } catch (const std::exception& scoped_ex) {
        std::fprintf(stderr,
                     "[diag] scoped CU open failed for %s: %s\n",
                     scoped.c_str(),
                     scoped_ex.what());
        std::fprintf(stderr, "[diag] trying exact CU name: %s\n", cu_name);
        std::fflush(stderr);
        try {
            ...
            std::fflush(stderr);
            return kernel;
        } catch (const std::exception& cu_ex) {
            std::string msg = "failed to open required CU '" +
                              std::string(cu_name) + "' for kernel '" +
                              kernel_name + "'. scoped error: " +
                              scoped_ex.what() + "; CU-name error: " +
                              cu_ex.what();
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        log_stage("q0 debug legend: d0 state (10=start, 70+phase=before send, 200=done), d1 active_iters, d2 30+phase=after stats read, d3 80+phase=after q0 send");
